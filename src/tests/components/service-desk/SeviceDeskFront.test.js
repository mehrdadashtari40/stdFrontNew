...
import $ from 'jquery';
import ServiceDeskFront from '../../../views/service-desk/components/ServiceDeskFront';
...
test('Make Snapshot file , ServiceDeskFrontComponent', () => {
	const wrapper = shallow(
		<Provider store={store}>
			<ServiceDeskFront match={mockData} />
		</Provider>
	)
		.dive({ context: { store } })
		.dive();
...
it('invokes `componentDidMount` when mounted', () => {
	jest.spyOn(ServiceDeskFront.prototype, 'componentDidMount');
	shallow(
		<Provider store={store}>
			<ServiceDeskFront match={mockData} />
		</Provider>
		...
		.dive({ context: { store } })
		.dive();
	expect(ServiceDeskFront.prototype.componentDidMount).toHaveBeenCalled();
	ServiceDeskFront.prototype.componentDidMount.mockRestore();
});
