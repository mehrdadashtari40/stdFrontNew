...
import {
    ...
    get_service_info,
    get_my_services,
    post_service_feedback, get_my_service_info
} from "../_redux/serviceDeskCrud";
import {GridLoader} from "react-spinners";
import {AppConfig} from "../../../appConfig";

function MyServiceDeskFront(props) {
    ...
    useEffect(() => {
        ...
        if (!state.isServicesLoaded) {
            get_my_services(config.apiServer,props.match.params.province ? props.match.params.province : "*").then(res => {
                res = res.map(x => {
                    x.categories = x.categories===undefined?[]:JSON.parse(x.categories);
        ...
        if (!state.isCategoriesLoaded) {
            get_my_categories(config.apiServer).then(res => {
                let cats = [];
                res.map(x => {
                            ...
                            <div className="row">
                                ...
                                <div className="col-md-9">
                                    <div className="service_table_index_bx first-step">
                                        ...
                                        {services.map(x => {
                                            if (x.id != 888)
                                                return <div key={x.id} className="col-xs-12 col-sm-4">
                                                    <div className="service_item_bx">
                                                        <a href={x.type === "0" ? (x.web.startsWith('http') ? x.web : 'http://' + x.web) : config.iframeServer + x.web + '?kid=' + x.id + '&pid=' + pid}
                                                           target={x.type === "0" ? "_self" : "_blank"}>
                                                            ...
                                                            <div>
                                                                <h3 className="service_name_style">{x.title}</h3>
    ...
    function itemCredential(id) {
        ...
        get_my_service_info(config.apiServer,id).then(res => {
...
export default compose(withTranslation(), connect(null, service.actions))(MyServiceDeskFront);